...
from aiohttp import __version__ as __aiohttp_version__
from ccctrl import (
    version_info,
    __version__,
)
...
# Keep-Alive Limit to prevent bug after ~ 40days (?!) TODO
MAX_ASYNCIO_SECONDS = 3456000

ccctrl_user_agent = f'ccctrl/{__version__} ' \
                     f'Python/{version_info[0]}.{version_info[1]} ' \
                     f'aiohttp/{__aiohttp_version__}'


class UAgents:
    SYSTEM = f'ccctrl/{__version__} Python/{version_info[0]}.{version_info[1]} aiohttp/{__aiohttp_version__}'
    REAL = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.182 Safari/537.36"

    @property
    def RANDOM(self) -> str:
        return choice(uagent.data_browsers[choice(uagent.data_randomize)])
